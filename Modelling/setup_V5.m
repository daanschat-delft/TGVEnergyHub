...
EnergyHouse = DemandEnergy;          % Household energy use for 1 year for owner-occupied houses in 2021 in NL [kWh] https://www.cbs.nl/en-gb/figures/detail/81528ENG
...
%% System sizing: The entire TGV Campus
SolarPanels             = 210;                             % [#] (445 Wp) 93325 kWp, 83810.33 kWh

DemandEnergy             = 99371.7;                         % [kWh]
Factor = round(DemandEnergy/2200);
FactorSmall = Factor/2;
SolarPanels             = 18*Factor;                       % [#] (445 Wp)

HydrogenCapacity        = 50*Factor;                           % [kg]
HydrogenStartCapacity   = 0.4*HydrogenCapacity;         % [kg]
...
% Fuel cell:
FuelCellPower       = 2380*FactorSmall;                   % [W] of electricity power (DC), is 1800 (AC) after inverter
FuelCellHyFlow      = 24*FactorSmall;                     % [NL/min] (at full load)
FuelCellStartPower  = 580*FactorSmall;                    % [W] of electricity power from start
FuelCellRampTime1   = 40;                     % [s] Ramp time from 0 to start power
FuelCellRampTime2   = 18*60;                  % [s] Ramp time from start power to full power
FuelCellRampDown    = 60;                     % [s] Ramp down from full power to 0 w

% Battery:
BatteryCapacity     = 15.36*Factor;                     % [kWh]
BatteryVNom         = 51.2;                             % [V]
BatteryInitialSoC   = 50;                               % [%]
BatteryCapacity     = 1000*BatteryCapacity/BatteryVNom; % [Ah]
GridExtra           = 0;                                % [W] Extra power to pull from the grid
...
% BoP:
WinterBase          = 235*FactorSmall;                    % [W]
SummerBase          = 80*FactorSmall;                     % [W]
FuelCellBoP         = 155*FactorSmall;                    % [W]
FuelCellAirBlower   = 6.4*33.9*FactorSmall;               % [W] draws 6.4 [A] directly from MPPT input
ElectrolyserBoP     = 80*FactorSmall;                     % [W] BoP


% EMS:
EMSElecStartSoC     = 30;                     % [%] Min SoC to start the electrolyser (was 80)
...
EMSFCTimeThresholds = [13, 14, 15, 16, 17];         % [13, 14, 15, 16, 17];      % [%] Range of time values to turn on FC
EMSFCSoCThresholds  = [.40, .50, .60, .70, .80];   % [40, 50, 60, 70, 80];      % [h] Range of SoC values to turn on FC
MinSoCGrid          = EMSFCStartSoC-5;
...
% Battery:
BatteryCapacity     = 2*15.36*10;    % [kWh]
BatteryVNom         = 51.2;     % [V]
BatteryInitialSoC   = 50;       % [%]
BatteryCapacity     = 1000*BatteryCapacity/BatteryVNom; % [Ah]
GridExtra           = 0;                % [W] Extra power to pull from the grid
...
SolarPanels             = 19;                           % [#] (445 Wp)
DemandEnergy             = 2200;                         % [kWh]
HydrogenCapacity        = 70;                           % [kg]
HydrogenStartCapacity   = 25;         % [kg]
...
%% System sizing Improved V6

SolarPanels             = 17;                           % [#] (445 Wp)
DemandEnergy             = 2200;                         % [kWh]
HydrogenCapacity        = 65;                           % [kg]
HydrogenStartCapacity   = 30;           % [kg]
...
% EMS:
EMSElecStartSoC     = 70;                     % [%] Min SoC to start the electrolyser (was 80)
EMSElecStartHr      = 16;                     % [h] Electrolyser starts before this hour (was 12)
EMSElecStopSoC      = 65;                     % [%] Min SoC to stop the electrolyser (was 65)
